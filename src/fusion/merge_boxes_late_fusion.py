...
if __name__ == "__main__":
    ...
    if (
            "lidar_ouster_south" in input_folder_path_target_boxes
            and "lidar_ouster_north" in input_folder_path_source_boxes
    ):
        ...
        transformation_matrix = np.linalg.inv(np.array(
            ...
            "lidar_ouster_north" in input_folder_path_target_boxes
            and "lidar_ouster_south" in input_folder_path_source_boxes
    ):
    ...
    for boxes_file_name_target, boxes_file_name_source in zip(input_files_target, input_files_source):
        ...
        for matches in matches_ids:
            ...
            if attribute is not None:
                object_3d_target["object_data"]["cuboid"]["attributes"]["text"].append(
                    {"name": "sensor_id", "val": "TrafficX_lidar_ouster_south_and_TrafficX_lidar_ouster_north"}
                )
            else:
                object_3d_target["object_data"]["cuboid"]["attributes"]["text"].append(
                    {"name": "sensor_id", "val": "TrafficX_lidar_ouster_south_and_TrafficX_lidar_ouster_north"}
        ...
        if "lidar_ouster_south" in boxes_file_name_target:
            output_file_name = boxes_file_name_target.replace(
                "lidar_ouster_south", "TrafficX_lidar_ouster_south_and_north_merged"
            )
        if "lidar_ouster_north" in boxes_file_name_target:
            output_file_name = boxes_file_name_target.replace(
                "lidar_ouster_north", "TrafficX_lidar_ouster_north_and_south_merged"
            )
        if "camera_basler_south1_8mm" in boxes_file_name_target:
            output_file_name = boxes_file_name_target.replace(
                "camera_basler_south1_8mm",
                "TrafficX_camera_basler_south1_8mm_and_south2_8mm_merged",
            )
