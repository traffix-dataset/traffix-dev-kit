...
def transform_base_to_lidar(boxes):
    ...
    transformation_matrix_TrafficX_base_to_lidar_south = np.array(
        [
    ...
    # transform boxes from TrafficX base frame to lidar frame
    boxes_transformed = np.dot(transformation_matrix_TrafficX_base_to_lidar_south, boxes_homogeneous.T).T
    return boxes_transformed[:, :3]


def transform_lidar_into_base(points):
    # transform data from south lidar to base
    transformation_matrix_lidar_south_to_TrafficX_base = np.array(
        [
    ...
    # transform point cloud into TrafficX base frame
    points_transformed = np.dot(transformation_matrix_lidar_south_to_TrafficX_base, points_homogeneous.T).T
