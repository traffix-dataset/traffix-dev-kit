...
class VisualizationUtils:
    ...
    def get_projection_matrix(self, camera_id, lidar_id, boxes_coordinate_system_origin):
        ...
        if camera_id == "camera_basler_south1_8mm" and lidar_id == "lidar_ouster_south":
            # R02 for TrafficX-I (intersection dataset)
            # R03 for TrafficX-C (cooperative dataset)
            # TODO: make this configurable and load from parameters
            release = "R03"
            if release == "R02":
                # projection matrix from lidar_ouster_south to TrafficX_camera_basler_south1
                # lidar->camera calibration
                # projection matrix for TrafficX-I dataset (R02 release)
        ...
        elif camera_id == "camera_basler_south2_8mm" and lidar_id == "lidar_ouster_south":
        ...
        elif camera_id == "camera_basler_south1_8mm" and lidar_id == "lidar_ouster_north":
            ...
            projection_matrix = np.matmul(intrinsic_camera_matrix, extrinsic_matrix_lidar_to_camera)
        elif camera_id == "camera_basler_south2_8mm" and lidar_id == "lidar_ouster_north":
            # projection matrix from lidar_ouster_north to TrafficX_camera_basler_south2
            intrinsic_camera_matrix = np.array(
            ...
            projection_matrix = np.matmul(intrinsic_camera_matrix, extrinsic_matrix)
        elif camera_id == "TrafficX_camera_basler_east_8mm" and lidar_id == "lidar_ouster_south":
            projection_matrix = np.array(
                [
        ...
        elif camera_id == "TrafficX_camera_basler_north_8mm" and lidar_id == "lidar_ouster_south":
        ...
        if boxes_coordinate_system_origin == "TrafficX_base" and lidar_id != "vehicle_lidar_robosense":
            if camera_id == "camera_basler_south1_8mm":
                # projection matrix from TrafficX_base to camera_basler_south1_8mm (camera to hd map calibration)
                projection_matrix = np.array(
                    [
            ...
            elif camera_id == "camera_basler_south2_8mm":
                projection_matrix = np.array(
                    [
                ...
                extrinsic_matrix_TrafficX_base_to_vehicle_camera = np.array([
                    [
                        -0.10853248450718671,
                        0.9802219683449281,
                        0.16548753616347203,
                ...
                projection_matrix = np.matmul(intrinsic_camera_matrix, extrinsic_matrix_TrafficX_base_to_vehicle_camera)
            else:
    ...
    def project_point_cloud_to_image(self, image, point_cloud, lidar_id, camera_id):
        ...
        projection_matrix = self.get_projection_matrix(camera_id, lidar_id,
                                                       boxes_coordinate_system_origin="lidar_ouster_south")
        ...
        if "openlabel" in box_data:
            for frame_id, frame_obj in box_data["openlabel"]["frames"].items():
                for object_id, label in frame_obj["objects"].items():
                    ...
                    # transform labels to TrafficX_base
                    ...
                    # transform detections from TrafficX_base to lidar
    ...
    def project_3d_position(self, position_3d):
        ...
        # TODO: load projection matrix from TrafficX dataset (label json files)
        # from TrafficX_base into camera_basler_south1_8mm
        projection_matrix = np.array(
            [
                [1599.6787257016188, 391.55387236603775, -430.34650625835917, 6400.522155319611],
                [-21.862527625533737, -135.38146150648188, -1512.651893582593, 13030.4682633739],
        ...
        # from TrafficX_base into camera_basler_south2_8mm (from camera-lidar calibration)
        # projection_matrix = np.array([
        #     [
        #         587.0282580208467,
        #         1104.2087993634066,
