...
##################################
# Evaluation Script for traffix 3D Object Detection
##################################
#
# Example usage:
...
def load_lidar_boxes_into_traffix(input_folder_path, object_min_points=0, ouster_lidar_only=False, prediction_type=None):
        ...
        if "openlabel" in json_data:
            for frame_id, frame_obj in json_data["openlabel"]["frames"].items():
                ...
                for object_id, label in frame_obj["objects"].items():
                    ...
                    # 3d position in traffix with z=0
                    position_3d_in_traffix_base = perspective.project_to_ground(image_pos)
                    orientation_vec = position_3d_in_traffix_base[:, 0] - position_3d_in_traffix_base[:, 1]
                    # yaw rotation in traffix_base frame
    ...
    for prediction_file in predictions_file_list:
        ...
        with open(prediction_file, "r") as f:
            for line in f:
                ...
                # 3d position in traffix_base with z=0
                position_3d_in_traffix_base = perspective.project_to_ground(image_pos)
                orientation_vec = position_3d_in_traffix_base[:, 0] - position_3d_in_traffix_base[:, 1]
                # yaw rotation in traffix_base frame
                rotation_yaw = np.arctan2(orientation_vec[1], orientation_vec[0])
                prediction[0] = position_3d_in_traffix_base[0, 0]
                prediction[1] = position_3d_in_traffix_base[1, 0]
                prediction[2] = position_3d_in_traffix_base[2, 0]
...
if __name__ == "__main__":
    ...
    if os.path.isfile(folder_path_ground_truth) and os.path.isfile(folder_path_predictions):
        ...
        gt_data = load_lidar_boxes_into_traffix(
            folder_path_ground_truth,
            object_min_points=object_min_points,
            ouster_lidar_only=use_ouster_lidar_only,
            prediction_type=prediction_type,
        ...
        if prediction_format == "openlabel":
            # load lidar labels into traffix for all prediction types (mono3d, lidar3d, multi3d)
            # parse openlabel predictions
            if prediction_type in ["lidar3d_supervised", "lidar3d_unsupervised"]:
                pred_data = load_lidar_boxes_into_traffix(
                    folder_path_predictions,
                    object_min_points=object_min_points,
                    ouster_lidar_only=use_ouster_lidar_only,
                    prediction_type=prediction_type,
