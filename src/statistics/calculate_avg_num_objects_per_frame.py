...
if __name__ == "__main__":
    ...
    plt.subplots_adjust(left=0.15, right=0.99, top=0.93, bottom=0.11)
    # ax = plt.gca()
    num_bins = 10
    ...
    for y_value, bin in zip(y_values, bins):
        ...
        # else:
        #     x_pos = bin + 1
        ax.text(bin+2, y_value + 10, str(y_value), color="black", fontweight="bold", fontsize=14)
    # show average
    plt.axvline(x=avg_value, linewidth=1, color="r", linestyle="--", zorder=3)
    plt.text(avg_value + 7, 260, str(round(avg_value)), color="r", fontsize=14)
    # show total number of labels
    plt.text(
        2,
        260,
        "Total 3D box labels:\n" + str(round(sum(num_labeled_objects_all_frames_grouped), 2)),
        color="black",
        fontsize=14,
    )
    ...
    plt.savefig(os.path.join(output_folder_path_statistic_plots, "box_plot_objects_in_frame.pdf"))
